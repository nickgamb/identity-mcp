#!/usr/bin/env python3
"""
Section Parser - Extract code blocks and content sections from conversations
============================================================================

This script parses ChatGPT conversation exports and extracts code blocks,
sections, and other structured content into separate files.

What it does:
  1. Reads a conversations.json file (from ChatGPT export)
  2. Scans messages for code blocks, section headers, and file markers
  3. Extracts each piece of content to its own file
  4. Creates a manifest tracking all extracted files

Detected patterns:
  - Code blocks: ```language ... ```
  - Section headers: ### 📄 `filename.txt` or SECTION N: Title
  - Path markers: ```language path=filename
  - Markdown files with titles

Usage:
  python parse_sections.py                      # Process all conversations  
  python parse_sections.py --id CONV_ID         # Process specific conversation
  python parse_sections.py --keywords foo bar   # Filter by keywords in title/content
  python parse_sections.py --min-lines 10       # Only extract sections > 10 lines
  python parse_sections.py --clean              # Delete output dir before extracting

Output:
  Creates parsed_sections/ directory with:
  - One folder per conversation (by ID)
  - Extracted files named by content or timestamp
  - extraction_manifest.json tracking all files
"""
...
def parse_conversations(conversations_file, target_conversation_id=None, keywords=None, min_lines=None):
    ...
    for conversation in conversations:
        ...
        # Create conversation-specific directory relative to project root
        base_output_dir = project_root / 'parsed_sections'
...
def parse_sections(input_file):
    ...
    output_dir = project_root / 'parsed_sections'
...
if __name__ == "__main__":
    ...
    parser.add_argument('conversations_file', help='Path to conversations.json (default: ../conversations/conversations.json)', 
                       nargs='?', default=None)
    ...
    if args.clean:
        script_dir = Path(__file__).parent
        project_root = script_dir.parent.parent
        outdir = project_root / 'parsed_sections'
        if outdir.exists():
            print('Deleting parsed_sections directory...')
            shutil.rmtree(outdir)

    # Use default conversations.json path if not provided
    conversations_file = args.conversations_file
    if conversations_file is None:
        script_dir = Path(__file__).parent
        project_root = script_dir.parent.parent
        conversations_file = project_root / 'conversations' / 'conversations.json'
        if not conversations_file.exists():
            print(f"Error: conversations.json not found at {conversations_file}")
            print("Please provide the path to conversations.json as an argument.")
            sys.exit(1)
