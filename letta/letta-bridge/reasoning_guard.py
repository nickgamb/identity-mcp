...
GUARD_SAMPLE_CHARS = int(os.getenv("LETTA_GUARD_SAMPLE_CHARS", "500"))
...
class ReasoningGuardState:
    ...
    total_chars: int = 0
    last_detection: Optional[LoopDetectionDetail] = None
...
class ReasoningLoopGuard:
    ...
    __slots__ = ("_buf", "state", "_last_pattern")
    ...
    @property
    def last_detection(self) -> Optional[LoopDetectionDetail]:
        return self.state.last_detection

    def note_suppressed(self, reason: LoopReason) -> LoopDetectionDetail:
        if not self.state.suppressed:
            self.state.suppressed = True
            self.state.close_reason = reason
            detail = self._build_detection_detail(reason)
            self.state.last_detection = detail
            return detail
        return self.state.last_detection or self._build_detection_detail(reason)
    ...
    def _truncate_sample(self, text: str) -> str:
        if len(text) <= GUARD_SAMPLE_CHARS:
            return text
        return text[-GUARD_SAMPLE_CHARS:]

    def _build_detection_detail(self, reason: LoopReason) -> LoopDetectionDetail:
        tail = self._buf[-REPETITION_BUF_SIZE:]
        sample_tail = self._truncate_sample(tail)
        pattern = self._last_pattern
        marker_count: Optional[int] = None

        if reason == LoopReason.PATTERN and pattern:
            sample = (
                f"Repeating pattern ({len(pattern)} chars, "
                f"{REPETITION_MIN_REPEATS}+ repeats):\n"
                f"{pattern!r}\n\n"
                f"Tail before detection:\n{sample_tail}"
            )
        elif reason == LoopReason.MARKERS:
            window = self._buf[-MARKER_WINDOW:]
            marker_count = len(_META_LINE_RE.findall(window))
            hits = _META_LINE_RE.findall(window)[-8:]
            sample = (
                f"Meta-token lines in last {MARKER_WINDOW} chars: {marker_count}\n"
                f"(threshold {MARKER_MIN_COUNT})\n\n"
                f"Examples:\n" + "\n".join(hits) + "\n\n"
                f"Tail:\n{sample_tail}"
            )
        else:
            sample = (
                f"Planning exceeded {MAX_REASONING_CHARS} chars "
                f"(got {self.state.total_chars}).\n\n"
                f"Tail:\n{sample_tail}"
            )

        return LoopDetectionDetail(
            reason=reason,
            sample=sample,
            total_chars=self.state.total_chars,
            pattern=pattern if reason == LoopReason.PATTERN else None,
...
def trim_reasoning_for_display(
    reasoning: str,
) -> tuple[str, bool, Optional[LoopDetectionDetail]]:
    """
    Non-streaming: walk reasoning and truncate if a loop is detected.
    Returns (safe_reasoning, was_trimmed, detection_detail).
    """
    if not reasoning:
        return "", False, None
    ...
    out: list[str] = []
    trimmed = False
    detail: Optional[LoopDetectionDetail] = None
    for line in reasoning.split("\n"):
        chunk = line + "\n"
        reason = guard.feed(chunk)
        if reason is not None:
            trimmed = True
            detail = guard.last_detection
            break
        out.append(chunk)
    return "".join(out).rstrip(), trimmed, detail
